...
"""
兼容 Python 3.8 + albumentations 1.4.18 的訓練腳本。
功能：
 - Sequential sliding crop dataset (預切片)
 - WeightedRandomSampler（強化含缺陷 patch）
...
import matplotlib.pyplot as plt  # <--- [新增] 導入繪圖庫
...
class CFG:
    ...
    PLOT_OUTPUT_PATH = "training_curves.png"  # <--- [新增] 訓練曲線圖儲存路徑
...
def plot_curves(history, save_path):
    ...
    # 繪製 IoU 曲線
    plt.subplot(1, 2, 2)
    plt.plot(epochs, history['val_defect_iou'], 'go-', label='Validation Defect IoU')
    plt.plot(epochs, history['val_mean_iou'], 'mo--', label='Validation Mean IoU')
...
def main():
    ...
    for epoch in range(CFG.EPOCHS):
        ...
        mean_iou = np.mean(list(val_iou.values()))
        history['train_loss'].append(train_loss)
        history['val_loss'].append(val_loss)
        history['val_defect_iou'].append(defect_iou)
    ...
    print("\nTraining finished. Best defect IoU:", best_defect_iou)

    # <--- [新增] 訓練結束後，呼叫繪圖函式 ---
    plot_curves(history, CFG.PLOT_OUTPUT_PATH)
